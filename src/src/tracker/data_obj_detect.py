...
class MOT16ObjDetect(torch.utils.data.Dataset):
    ...
    def __init__(self, root, transforms=None, vis_threshold=0.25):
        ...
        for f in listdir_nohidden(root):
            ...
            for i in range(1, seq_len + 1):
                img_path = os.path.join(_imDir, f"{i:06d}{im_ext}") #e.g: <...>\MOT16\train\MOT16-02\img1\000003.jpg
                assert os.path.exists(img_path), \
                    'Path does not exist: {img_path}'
                # self._img_paths.append((img_path, im_width, im_height))
                self._img_paths.append(img_path) #contain image paths of all images in train folder 
    ...
    def _get_annotation(self, idx):
        ...
        file_index = int(os.path.basename(img_path).split('.')[0]) #index of that image. 
        ...
        with open(gt_file, "r") as inf:
            ...
            for row in reader:
                ...
                if int(row[0]) == file_index and int(row[6]) == 1 and int(row[7]) == 1 and visibility >= self._vis_threshold:
                    ...
                    bounding_boxes.append(bb) #bounding_boxes is a list of dictionary, each dictionary is a bounding box in that image. 

        num_objs = len(bounding_boxes) #bounding_boxes consists of all bounding box for one image. 

        boxes = torch.zeros((num_objs, 4), dtype=torch.float32)
        visibilities = torch.zeros((num_objs), dtype=torch.float32)
        
        #turn the bounding boxes into one tensor of bounding box 
        ...
        return {'boxes': boxes,
                'labels': torch.ones((num_objs,), dtype=torch.int64), #only human labels 
                'image_id': torch.tensor([idx]),
                'area': (boxes[:, 3] - boxes[:, 1]) * (boxes[:, 2] - boxes[:, 0]), #areas of all boxes. 
                'iscrowd': torch.zeros((num_objs,), dtype=torch.int64),
    ...
    def __getitem__(self, idx):
        ...
        return img, target #target is a dictionary of {boxes, labels, image_id, area, iscrowd, visibilities}, img is Image object 
