...
class MOT16Sequences():
    ...
    def __init__(self, dataset, root_dir, **kwargs):
        ...
        self._data = _sets[dataset](root_dir, **kwargs) #call the lambda function , self._data is MOT16 object

    def __len__(self):
        return len(self._data) #return number of sequence

    def __getitem__(self, idx):
        return self._data[idx] # return sequence based on index. E.g:Each sequence is a MOT16Sequence object. E.g: MOT16-02 is is a MOT16Sequence object.  
...
class MOT16(Dataset):
    ...
    def __init__(self, root_dir, split, **kwargs):
        ...
        train_sequences = list(listdir_nohidden(
            os.path.join(root_dir, 'train'))) #MOT16-02, MOT16-04,...
    ...
    def __getitem__(self, idx):
        return self._data[idx] #return a MOT16Sequence object 
...
class MOT16Sequence(Dataset):
    ...
    def __init__(self, root_dir, seq_name, vis_threshold=0.0, load_seg=False):
        ...
        self._load_seg = load_seg #load_seq = True
        ...
        self.data, self.no_gt = self._sequence() #len = seqLength, list of dict {gt, img_path, vis, seg_img}
    ...
    def _sequence(self):
        seq_name = self._seq_name # seq_name (string): Sequence to take, e.g: 'MOT16-02'
        ...
        seqLength = int(config['Sequence']['seqLength'])  #e.g: seqLength = 600
        img_dir = config['Sequence']['imDir'] #image directory, where store images

        img_dir = osp.join(seq_path, img_dir) #<..>\cv3dst_exercise\data\MOT16\train\MOT16-02\img1
        gt_file = osp.join(seq_path, 'gt', 'gt.txt') #<..>\cv3dst_exercise\data\MOT16\train\MOT16-02\gt
        seg_dir = osp.join(seq_path, 'seg_ins')#<..>\cv3dst_exercise\data\MOT16\train\MOT16-02\seg_ins: image segmentation

        data = [] #len = seqLength, list of dict, each dict consists of element of boxes, visibility, seg_imgs and image file
        boxes = {} #len = seqLength
        visibility = {}#len = seqLength
        seg_imgs = {} #len =seqLength
        ...
        if osp.exists(gt_file):
            with open(gt_file, "r") as inf:
                ...
                for row in reader: #format of gt file:position 0,1,2,3,4,5 =  sequence index(1-600),box index(1-83)( different box correspond to different people),x of top left, y of top left, width, height 
                    # position 6,7,8: class person(0-1 person or not), certainity 1(sure that is person), visibility(occlusion) >= 0.25
                    if int(row[6]) == 1 and int(row[7]) == 1 and float(row[8]) >= self._vis_threshold: #only choose bounding box that correspond to person 
        ...
        if self._load_seg:
            if osp.exists(seg_dir):
                for seg_file in listdir_nohidden(seg_dir):
                    ...
                    seg_img = Image.open(
                        osp.join(seg_dir, seg_file))# read segmentation file as whole image , PIL.PngImagePlugin.PngImageFile object
                    seg_imgs[frame_id] = seg_img

        for i in range(1, seqLength + 1):
            img_path = osp.join(img_dir, f"{i:06d}.jpg")

            datum = {'gt': boxes[i], #boxes[i] is a dict , all boxes in a frame
                     'im_path': img_path,
                     'vis': visibility[i]}
